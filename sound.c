...
void music_display(void)
{
 ...
 for (i = 0; i < MBOX_X + 1; i ++)
 {
  vline(display[0], MB_X + MB_SIZE*i, MB_Y, MB_Y + MBOX_Y * MB_SIZE, COL_BOX1);
 }
 for (j = 0; j < MBOX_Y + 1; j ++)
 {
  hline(display[0], MB_X, MB_Y + MB_SIZE*j, MB_X + MBOX_X * MB_SIZE, COL_BOX1);
 }
 ...
 for (ch = 0; ch < CHIPS; ch ++)
 {
  ...
  switch(chip [ch])
  {
   case CH_LINE: col = COL_EBOX3; break;
   case CH_SAW: col = COL_EBOX2; break;
  }
  rectfill(display[0], MB_X+1 + chip_x [ch] * MB_SIZE, MB_Y+1 + chip_y [ch] * MB_SIZE,
                       MB_X-1 + (chip_x[ch]+1) * MB_SIZE, MB_Y-1 + (chip_y [ch]+1) * MB_SIZE, col);
  x = MB_X+1 + chip_x [ch] * MB_SIZE + (MB_SIZE>>1);
  y = MB_Y+1 + chip_y [ch] * MB_SIZE + (MB_SIZE>>1);
  int angle = 0;
  switch(chip_dir [ch])
  {
   case CD_UP: angle = -ANGLE_4; break;
   case CD_RIGHT: angle = 0; break;
   case CD_LEFT: angle = ANGLE_2; break;
   case CD_DOWN: angle = ANGLE_4; break;
  }

  triangle(display[0], x + xpart(angle, 6), y + ypart(angle, 6),
   x + xpart(angle + ANGLE_4 + ANGLE_16, 6), y + ypart(angle + ANGLE_4 + ANGLE_16, 6),
   x + xpart(angle - ANGLE_4 - ANGLE_16, 6), y + ypart(angle - ANGLE_4 - ANGLE_16, 6),
   COL_BOX2);
 }
